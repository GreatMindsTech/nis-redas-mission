# Simple test script to verify AWS Labs Frontend MCP Server setup
...
try {
    ...
    if ($output) {
        ...
        Write-Host "Package may not be in tool list, but dependencies were installed" -ForegroundColor Yellow
    }
} catch {
    Write-Host "Could not verify package installation" -ForegroundColor Yellow
}
Write-Host ""
...
Write-Host "Test 5: Testing server startup..." -ForegroundColor Yellow
...
try {
    ...
    if (!$process.HasExited) {
        ...
        Write-Host "Server process exited quickly" -ForegroundColor Yellow
        Write-Host "This is normal for --help commands" -ForegroundColor Cyan
    }
} catch {
    Write-Host "Could not test server startup: $_" -ForegroundColor Yellow
}
Write-Host ""
...
Write-Host "1. Connect your MCP client" -ForegroundColor Gray
